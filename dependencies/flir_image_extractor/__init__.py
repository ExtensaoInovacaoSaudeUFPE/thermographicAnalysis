...
class FlirImageExtractor:
    ...
    def plot(self):
        ...
        thermal_height, thermal_width = thermal_np.shape[:2]
        ...
        cropped_rgb_image = rgb_np[y_offset:392, x_offset:548]
        ...
        # Plotar as imagens
        ax[0].imshow(thermal_np, cmap='hot')
        ...
        def update_temp_text(event):
            if not event.inaxes or event.inaxes == ax[1]: #Quando o mouse estiver sobre a imagem RGB recortada
                ...
                temperature = thermal_np[y1,x1] # O valor da temperatura na posição x1,y1 (acurácia validada)
